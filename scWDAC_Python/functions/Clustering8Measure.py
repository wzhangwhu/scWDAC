...
def Clustering8Measure(Y, predY):
    ...
    n = len(np.asarray(Y).ravel(order='F'))
    ...
    correnum = 0
    for ci in range(1, pred_classnum + 1):
        incluster = Y[np.where((predY == predLidx[ci - 1]).ravel())[0]]

        if incluster.size == 0:
            inclunub = np.array([0], dtype=float)
        else:
            bins = np.arange(1, int(np.max(incluster)) + 1)
            inclunub, _ = np.histogram(incluster, bins=np.arange(1, int(np.max(incluster)) + 2))
        correnum = correnum + np.max(inclunub)
    Purity = correnum / len(np.asarray(predY).ravel(order='F'))

    res = bestMap(Y, predY)

    ACC = len(np.where((Y == res).ravel())[0]) / len(np.asarray(Y).ravel(order='F'))

    MIhat = MutualInfo(Y, res)

    Fscore, Precision, Recall = compute_f(Y, predY)
    A, nmi, Entropy           = compute_nmi(Y, predY)
    AR                       = RandIndex(Y, predY)[0]

    result = np.array([ACC, nmi, Fscore, Precision, AR, Purity, Recall, Entropy], dtype=float)
...
def MutualInfo(L1, L2):
    ...
    PPP = (
        P12 /
        np.tile(P2.reshape(1, -1), (nClass, 1)) /
        np.tile(P1.reshape(-1, 1), (1, nClass))
    )
...
def hminired(A):
    ...
    for k in range(1, n + 1):
        ...
        A[k - 1, np.concatenate(([n + 1], idx_cols)) - 1] = np.concatenate(
            (-idx_cols, np.array([0], dtype=int))
        )
